...
def main(page: ft.Page):
    ...
    def delete_last(_):
        if greeting_history:
            ...
            history_text.value = "Greetings:\n" + "\n".join(greeting_history) if greeting_history else "Greetings:"
        else:
            text_hello.value = "Empty history!"
            text_hello.color = ft.Colors.ORANGE
        page.update()
    ...
    delete_last_button = ft.ElevatedButton("Delete last", icon=ft.Icons.UNDO, color=ft.Colors.YELLOW, icon_color=ft.Colors.AMBER_200, on_click=delete_last)

    main_objects = ft.Row([name_input, elevated_button, delete_last_button, delete_button, theme_button], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
